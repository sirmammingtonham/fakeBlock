...
class TFFakeNewsOutput(ModelOutput):
    loss: Optional[tf.Tensor] = None
    logits: Optional[tf.Tensor] = None
    logits_aggregate: Optional[tf.Tensor] = None
    logits_category: Optional[tf.Tensor] = None
...
class TFDistilBertForFakeNewsClassification(TFDistilBertPreTrainedModel):
    ...
    def call(
        ...
        logits_aggregate = self.classifier_aggregate(
            pooled_output)  # (bs, dim)
        ...
        if inputs["labels_aggregate"] is not None and inputs["labels_category"] is not None:
            loss_aggregate = self.compute_loss(
                inputs["labels_aggregate"], logits_aggregate)
            loss_category = self.compute_loss(
                inputs["labels_category"], logits_category)
            loss = loss_aggregate + loss_category
            # print(loss)

        if not inputs["return_dict"]:
            output = (logits_aggregate, logits_category) + \
                distilbert_output[1:]
            return ((loss,) + output) if loss is not None else output

        if loss is None:  # for savedmodel output
            return TFFakeNewsOutput(
                logits_aggregate=logits_aggregate,
                logits_category=logits_category,
            )

        return TFFakeNewsOutput(
            loss=loss,
            logits=logits_aggregate,
            hidden_states=distilbert_output.hidden_states,
            attentions=distilbert_output.attentions,
        )

    def serving_output(self, output: TFFakeNewsOutput) -> TFFakeNewsOutput:
        ...
        return TFFakeNewsOutput(
            logits_aggregate=output.logits_aggregate,
            logits_category=output.logits_category,
            hidden_states=hs,
            attentions=attns
        )
